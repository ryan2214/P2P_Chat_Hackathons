...
def register(ip_address, username, password):
    c.execute('INSERT INTO user (ip_address, username, password, isOnline) VALUES (?, ?, ?, 1)',
...
def user_login(username):
    c.execute('INSERT INTO user(isOnline) VALUES (1) WHERE username = ?', (username,))
    c.commit() 

def user_logout(username):
    c.execute('INSERT INTO user(isOnline) VALUES (0) WHERE username = ?', (username,))
    c.commit() 
